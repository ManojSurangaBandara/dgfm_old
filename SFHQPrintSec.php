...
<head>
	...
	function getbranchtothispage(branch_id,status){
	document.location.href="SFHQPrintSec.php?branch_id="+branch_id+"&status="+status;                                 
	}	
	</script>
...
<select name="branch_id" class="ComboBoxcesSmall" id="branch_id" onchange="getbranchtothispage(this.value,'<?php echo $status; ?>')"style="width:100px;">
...
<?php   	  
	...
	if (!$max_rec) 
		...
		if($branch_id ==6){
			...
			$result = Projects :: GetBillsofSFHQ($status,$search,$user_type_id);
			if (!$result) return;
			...
			$result = Projects :: SFHQBilltoPrint($status,$search,$user_type_id,$sfhq_id,$current1, $length);	
			if (!$result) return;

			}
			else{
				$result = Projects :: SFHQPrintingBills($branch_id,$status,$search,$user_type_id,$sfhq_id,$current1, $length);				
				if (!$result) return;	
                 ...
                 <?php }?>  
				  ...
				  <?php 
						...
						{ ?>                        
                         ...
                         <td class="last"><a href="ViewSfhqBillDetails.php?billId=<?php echo $row[0]; ?>&branch_id=<?php echo $row[11]; ?>&status=<?php echo $status; ?>" class="link"><img src="images/Play-Normal.ico" width="16" height="16" alt="" /></a></td>                        
                         <td class="last"><a href="ChequeDetails.php?projectid=<?php echo $row[0]; ?>&branch_id=<?php echo $branch_id; ?>" class="link"><img src="images/print.png" width="16" height="16" alt="" /></a></td>
	...
	?>	
		...
		<tr><td align="right">
			<?php
				...
				?>
				<a href="SFHQPrintSec.php?&page=<?php echo($page_id - 1); ?>&branch_id=<?php echo $branch_id; ?>&status=<?php echo $status; ?>&max=<?php echo($max_rec); ?>">&laquo; Previous</a> | <?php } ?>
				<?php echo((($page_id * $limit) + 1));?> - <?php echo((($page_id * $limit) + $num_rows)); ?> of <?php echo($max_rec); ?>
				<?php if ((($page_id * $limit) + $limit) < $max_rec) { ?>
				| <a href="SFHQPrintSec.php?page=<?php echo($page_id + 1); ?>&branch_id=<?php echo $branch_id; ?>&status=<?php echo $status; ?>&max=<?php echo($max_rec)?>">Next &raquo</a>
