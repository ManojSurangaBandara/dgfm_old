...
<head>
	...
	document.location.href="ResetPassword.php?branch_id="+branch_id+"&status="+status;                                 
	}
	
	function getstatus(status,branch_id){
	document.location.href="ResetPassword.php?status="+status+"&branch_id="+branch_id;                                 
	}
	</script>
    
    <script type="text/javascript">
	
	function resetnow(id){
	var message = confirm("Are you sure you want to reset password?")	
	if(message==true)	
	document.location.href="controller/users.controller.php?mode=reset&id="+id;
	}
			...
			<div class="top-bar">
			 <!-- <a href="NewAccountCreation.php" class="button">ADD NEW </a> -->
              ...
              <!--  <a href="add_progress_report.php" class="button_r">PROGRESS REPORT </a>-->
				<h1>Reset Password for the Year of <?php echo $_SESSION['log_year'];  ?></h1>
